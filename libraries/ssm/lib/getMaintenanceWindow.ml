...
let of_http body =
  try
    ...
    try
      Util.or_error (Util.option_bind resp GetMaintenanceWindowResult.parse)
        (let open Error in
           BadResponse
             {
               body;
               message =
                 "Could not find well formed GetMaintenanceWindowResult."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing GetMaintenanceWindowResult - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
