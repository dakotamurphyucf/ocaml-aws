...
let of_http body =
  try
    ...
    let resp =
      Xml.member "DescribeMaintenanceWindowScheduleResponse" (snd xml) in
    try
      Util.or_error
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing DescribeMaintenanceWindowScheduleResult - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
