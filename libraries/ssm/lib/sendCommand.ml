...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append [("Version", ["2014-11-06"]); ("Action", ["SendCommand"])]
         (Util.drop_empty
...
let of_http body =
  try
    ...
    try
      Util.or_error (Util.option_bind resp SendCommandResult.parse)
        (let open Error in
           BadResponse
             {
               body;
               message = "Could not find well formed SendCommandResult."
             })
    with
