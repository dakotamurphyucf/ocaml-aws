      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render
                  (RegisterDefaultPatchBaselineRequest.to_query req))))) in
  (`POST, uri, [])
let of_http body =
  try
           ...
           BadResponse
             {
               body;
               message =
                 "Could not find well formed RegisterDefaultPatchBaselineResult."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing RegisterDefaultPatchBaselineResult - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
